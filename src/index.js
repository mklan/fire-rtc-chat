  ...
  initializeFireRTC: ({ id, fireBaseConfig}) => async (state, actions) => {
    ...
    try {
      ...
      await firebase.initializeApp(fireBaseConfig, 'fireRTCChat');
        ...
        actions.setState({ error: 'Failed to connect to Firebase, please check your settings', loading: false, link: null });
    }
    ...
    fireRTC = createFireRTC({
      ...
      onError: error => actions.setState({ error: 'Failed to connect! check your internet connection and try again', loading: false, link: null }),
...
const MainView = () => (state, actions) =>  (
  <div oncreate={actions.loadSettings} className="main-view">
    ...
    { state.error && <Error error={state.error} onClose={() => actions.setState({ error: null })} />}
    ...
    { state.isInitiator ? <div>
      ...
      <DecryptionForm buttonLabel={'Enter chat'} encryption={URLparams.get('code')} onDecrypt={actions.initializeFireRTC} onError={() => actions.setState({ error: 'Error decrypting'})}/>
